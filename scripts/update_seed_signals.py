...
def classify_trade_layer(row: dict[str, object]) -> tuple[str, list[str]]:
    """
    將原始訊號分為三層：
      - official_trade：可正式交易（A 級分數、風險位置合理、方向 Taker 共振、流動性足夠）
      - warning：僅警示，不建議直接下單
    ...
    timeframe = str(row.get("timeframe") or snapshot.get("interval") or "").lower()
    side = str(row.get("signal_type") or "")
    is_long = side == "reversal_bullish"
    is_short = side == "reversal_bearish"
    taker = raw_number(metric_value(row, "taker_buy_ratio", 0), 0)
    has_15m_alignment = any(
        metric_value(row, key) in (True, "true", 1, "1")
        for key in ("mtf_15m_confluence", "mtf_15m_oi_confluence")
    )
    ...
    if score < 75:
        reasons.append("score_below_75_a_grade_required")
    # --- 進場與風險 ---
    if entry <= 0 or sl <= 0:
        blocks.append("missing_entry_or_sl")  # 缺少進場價或止損，硬性過濾
    elif risk < 0.8 or risk > 3.5:
        reasons.append("risk_distance_outside_0_8_to_3_5_pct")
    ...
    if price_move > 2:
        reasons.append("chasing_price_move_over_2_pct")
    # --- 方向與週期確認 ---
    if not taker:
        reasons.append("taker_confirmation_missing")
    elif (is_long and taker < 0.54) or (is_short and taker > 0.46):
        reasons.append("taker_not_directionally_confirmed")
    if timeframe in ("5m", "5min", "5") and not has_15m_alignment:
        reasons.append("five_minute_requires_15m_alignment")
    # --- CVD 背離只作預警，不能單獨形成正式進場 ---
    if "cvd_5m" in setup or "divergence" in setup:
        reasons.append("cvd_divergence_warning_only_requires_15m_structure")
    if blocks:
